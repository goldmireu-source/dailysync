...
def _build_prompt(post: TechPost) -> str:
    if post.body and post.body_status == "success":
        ...
        depth_note = (
            ...
            "용어 등)·결과를 빠짐없이 반영하세요. 한 포인트에 여러 내용을 욱여넣지 말고 "
            "포인트를 여러 개로 나누세요 — 카드 한 장(포인트 하나)은 짧게 유지하고, 담을 "
            "내용이 많으면 포인트 개수를 늘려서(최대 8개까지) 다음 카드로 넘기는 방식입니다."
        )
    else:
...
{{
  "key_points": [
    "핵심 포인트 1 — 구체적인 기술·수치·용어를 포함하되 한 문장으로 짧게",
    "핵심 포인트 2",
    "핵심 포인트 3",
    "... 내용이 더 있으면 계속 (최대 8개)"
  ],
