...
- `04:00` daily — `job_cleanup_old_data` (`jobs/cleanup.py`): deletes Article/Cluster/Paper rows older than 4 days by `published_at`. **Saved items are preserved**: any `Cluster.saved_at IS NOT NULL` keeps the cluster + ALL its articles regardless of age; any `Paper.saved_at IS NOT NULL` is kept too. Default retention is 4 days; pass `retention_days=N` to `job_cleanup_old_data()` for ad-hoc runs.
